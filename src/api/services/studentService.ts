...
export async function getStudentLabs(semesterNumber: number = 5) {
  const db = await getDb();
  const labs = await db
    .collection("laboratories")
    .find({ semesterNumber, isActive: true })
    .toArray();
...
export async function getLabExercises(data: { labId?: string; labCode?: string }) {
  const db = await getDb();
...
export async function submitExerciseWork(data: { studentId?: string; exerciseId: string; code: string; language: string }) {
  const db = await getDb();
  const subDoc = {
